    ...
    if not target_table or header_row_idx == -1 or discuss_row_idx == -1:
        ...
        if is_overflow:
            ...
            for chunk_idx, chunk_text in enumerate(chunks_to_move_to_new_pages):
                # 每頁 10 塊或在第一個溢出塊時添加新頁
                if chunk_idx % 10 == 0:
                    doc.add_page_break()
                    
                    # 創建一個與原始交班表格具有相同結構的新表格
...
if st.button("🚀 生成下載 Word", type="primary"):
    try:
        ...
        if overflow:
            st.info("ℹ️ 交班內容較長，系統已自動為您將『討論與講評』及『簽章區塊』移至新分頁！")
        else:
            st.success("✅ 檔案已更新並備妥！")
